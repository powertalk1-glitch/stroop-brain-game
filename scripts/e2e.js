...
(async () => {
  ...
  await page.click('#enter-setup');
  assert.ok((await page.locator('#profile-label').textContent()).includes('成人混合挑戰'));
  await page.click('[data-difficulty="hard"]');
  assert.ok(await page.locator('body').evaluate((el) => el.classList.contains('profile-adult')));
  await page.click('#start-game');
  await page.waitForTimeout(3000);
  assert.ok(await page.locator('#game-screen').evaluate((el) => el.classList.contains('active')));
  assert.equal(await page.locator('.answer').count(), 6);
  assert.equal(await page.locator('.arena').evaluate((el) => getComputedStyle(el).backgroundColor), 'rgb(118, 118, 118)');
  ...
  assert.equal(await page.locator('.answer:not(:disabled)').count(), 6);
  ...
  assert.equal(await mobile.locator('[data-profile]').count(), 0);
  assert.equal(await mobile.locator('#enter-setup').count(), 1);
  await mobile.screenshot({ path: 'mobile-home.png', fullPage: true });
  await mobile.click('#enter-setup');
  await mobile.click('#start-game');
  await mobile.waitForTimeout(3000);
  assert.equal(await mobile.locator('.answer').count(), 6);
  const gameDimensions = await mobile.evaluate(() => ({ viewport: innerWidth, page: document.documentElement.scrollWidth }));
  assert.ok(gameDimensions.page <= gameDimensions.viewport, `手機遊戲水平溢出：${gameDimensions.page} > ${gameDimensions.viewport}`);
  await mobile.screenshot({ path: 'mobile-game.png', fullPage: true });
